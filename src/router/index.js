...
function addDynamicMenuAndRoutes(userName, to, from) {
  ...
  api.menu
    .findNavTree({ loginid: userName })
    .then(res => {
      // 添加动态路由
      api.user.findPermissions(userName).then(res => {
        // 保存用户权限标识集合
        store.commit("setPerms", res.data);
      });
      if (res.status == 200) {
        let dynamicRoutes = addDynamicRoutes(res.data);
        //router.options.routes[0].component= Home
...
function handleIFrameUrl(path) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    let URL = path + "/";
    if (
      (path != null && path.includes(iframe.path)) ||
      URL.includes(iframe.path)
    ) {
      url = iframe.url;
